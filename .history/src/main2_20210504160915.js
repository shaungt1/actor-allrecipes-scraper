...
function extractData(request, $) {
    ...
    return {
        ...
        calories: $('[itemprop=calories]').length > 0
            ...
            // Add nutrient data to set
            protein : $('[itemprop=protein ]').length > 0
            ? (() => {
                try {
                    return $('[itemprop=protein ]').text().split(' ')[0];
                } catch (e) {
                    return '';
                }
            })()
            : (() => {
                try {
                    return $('.recipe-nutrition-section .section-body').text().trim().match(/(\d+) protein/)[2];
                } catch (e) {
                    return '';
                }
            })(),
